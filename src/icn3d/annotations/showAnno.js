...
class ShowAnno {
    ...
    showAnnotations() { var ic = this.icn3d, me = ic.icn3dui;
        var thisClass = this;
        ...
        if((ic.bAssemblyNote === undefined || !ic.bAssemblyNote) && ic.asuCnt !== undefined ) {
            var html = "     <br><div id='" + ic.pre + "assembly_note' style='margin-left:5px;'><span class='icn3d-annoLargeTitle'>Assembly Tips:</span> Only the asymmetric unit is shown in the sequence window.<br>Click \"Assembly\" in the menu \"View\" to switch between asymmetric unit and biological assembly(<b>" + ic.asuCnt + "</b> asymmetric unit).</div>";
        ...
        if(ic.bAnnoShown === undefined || !ic.bAnnoShown) {
            ...
            var dialogWidth =(ic.icn3dui.cfg.notebook) ? ic.icn3dui.htmlCls.WIDTH / 2 : $("#" + ic.pre + "dl_selectannotations").dialog( "option", "width" );
            ...
            for(var i = 0, il = chainArray.length; i < il; ++i) {
                ...
                var atom = ic.firstAtomObjCls.getFirstCalphaAtomObj(ic.chains[chainArray[i]]);
                  ...
                  &&(ic.proteins.hasOwnProperty(atom.serial) || ic.nucleotides.hasOwnProperty(atom.serial)) ) {
                    for(var r = 0, rl = ic.chainsSeq[chainArray[i]].length; r < rl; ++r) {
                        ...
                        if(resObj.name !== '' && resObj.name !== '-' && resObj.name == resObj.name.toUpperCase()) {
                            ...
                            var atom = ic.firstAtomObjCls.getFirstCalphaAtomObj(ic.residues[resid]);
                            if(atom === undefined) atom = ic.firstAtomObjCls.getFirstAtomObj(ic.chains[chainArray[i]]);
            ...
            if(ic.icn3dui.cfg.blast_rep_id === undefined) {
               ...
               var url = ic.icn3dui.htmlCls.baseUrl + 'pwaln/pwaln.fcgi?from=querytarget';
               var dataObj = {'targets': ic.icn3dui.cfg.blast_rep_id, 'queries': ic.icn3dui.cfg.query_id}
               if(ic.icn3dui.cfg.query_from_to !== undefined ) {
                   // convert from 1-based to 0-based
                   var query_from_to_array = ic.icn3dui.cfg.query_from_to.split(':');
               ...
               if(ic.icn3dui.cfg.target_from_to !== undefined) {
                   // convert from 1-based to 0-based
                   var target_from_to_array = ic.icn3dui.cfg.target_from_to.split(':');
    ...
    showAnnoSeqData(nucleotide_chainid, chemical_chainid, chemical_set) { var ic = this.icn3d, me = ic.icn3dui;
    ...
    getAnnotationData() { var ic = this.icn3d, me = ic.icn3dui;
        var thisClass = this;
        ...
        for(var chnid in ic.protein_chainid) {
            ...
            var fullProteinName = ic.showSeqCls.getProteinName(chnid);
            ...
            var chainHtml = "<div id='" + ic.pre + "anno_" + chnid + "' class='icn3d-annotation'>" + categoryStr
        ...
        var url = ic.icn3dui.htmlCls.baseUrl + "/vastdyn/vastdyn.cgi?chainlist=" + chnidBaseArray;
        ...
        if(ic.chainid_seq !== undefined) {
            ...
            $.ajax({
              ...
              error : function(xhr, textStatus, errorThrown ) {
                ...
                for(var chnid in ic.protein_chainid) {
                    var chnidBase = ic.protein_chainid[chnid];
    ...
    getSequenceData(chnid, chnidBase, type, index) { var ic = this.icn3d, me = ic.icn3dui;
        var fullProteinName = ic.showSeqCls.getProteinName(chnid);
        var proteinName = fullProteinName;
        if(proteinName.length > 40) proteinName = proteinName.substr(0, 40) + "...";
        var categoryStr = "";
    ...
    getCombinedSequenceData(name, residArray, index) { var ic = this.icn3d, me = ic.icn3dui;
        ...
        var sid =(ic.icn3dui.cfg.mmdbid !== undefined && ic.chainid2sid !== undefined) ? ic.chainid2sid[firstChainid] : undefined;
        if(sid !== undefined) {
        ...
        var htmlTmp = '<div id="' + ic.pre + 'giseq_sequence" class="icn3d-dl_sequence">';
        ...
        var color = ic.icn3dui.htmlCls.GREY8;
        ...
        var width = Math.round(ic.seqAnnWidth * residArray.length / ic.maxAnnoLength);
        if(width < 1) width = 1;
    ...
    processSeqData(chainid_seq) { var ic = this.icn3d, me = ic.icn3dui;
        for(var chnid in ic.protein_chainid) {
            var chnidBase = ic.protein_chainid[chnid];
            ...
            if(ic.icn3dui.cfg.blast_rep_id != chnid) {
              ...
              var data = ic.seqStructAlignData;
              ...
              if(query !== undefined && target !== undefined) {
                  ...
                  for(var i = 0, il = targetSeq.length; i < il; ++i) {
                      ...
                      if(ic.targetGapHash.hasOwnProperty(i)) {
                          for(var j = ic.targetGapHash[i].from; j <= ic.targetGapHash[i].to; ++j) {
                              text += querySeq[j];
                          }
                      }
                      ...
                      if(target2queryHash.hasOwnProperty(i) && target2queryHash[i] !== -1) {
                          ...
                          var colorHexStr = this.getColorhexFromBlosum62(targetSeq[i], querySeq[target2queryHash[i]]);
              ...
              if(ic.consrvResPosArray !== undefined) {
                for(var i = 0, il = ic.consrvResPosArray.length; i < il; ++i) {
    ...
    enableHlSeq() { var ic = this.icn3d, me = ic.icn3dui;
    ...
    getAnDiv(chnid, anno) { var ic = this.icn3d, me = ic.icn3dui;
    ...
    addButton(chnid, classvalue, name, desc, width, buttonStyle) { var ic = this.icn3d, me = ic.icn3dui;
    ...
    addSnpButton(snp, classvalue, name, desc, width, buttonStyle) { var ic = this.icn3d, me = ic.icn3dui;
    ...
    conservativeReplacement(resA, resB) { var ic = this.icn3d, me = ic.icn3dui;
        var iA =(me.parasCls.b62ResArray.indexOf(resA) !== -1) ? me.parasCls.b62ResArray.indexOf(resA) : me.parasCls.b62ResArray.length - 1; // or the last one "*"
        var iB =(me.parasCls.b62ResArray.indexOf(resB) !== -1) ? me.parasCls.b62ResArray.indexOf(resB) : me.parasCls.b62ResArray.length - 1; // or the last one "*"
        var matrixValue = me.parasCls.b62Matrix[iA][iB];
        if(matrixValue > 0) {
            return true;
        }
        else {
    ...
    getColorhexFromBlosum62(resA, resB) { var ic = this.icn3d, me = ic.icn3dui;
        var iA =(me.parasCls.b62ResArray.indexOf(resA) !== -1) ? me.parasCls.b62ResArray.indexOf(resA) : me.parasCls.b62ResArray.length - 1; // or the last one "*"
        var iB =(me.parasCls.b62ResArray.indexOf(resB) !== -1) ? me.parasCls.b62ResArray.indexOf(resB) : me.parasCls.b62ResArray.length - 1; // or the last one "*"
        var matrixValue = me.parasCls.b62Matrix[iA][iB];
