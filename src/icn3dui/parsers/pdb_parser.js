...
iCn3DUI.prototype.loadPdbData = function(data, pdbid, bOpm) { var me = this, ic = me.icn3d; "use strict";
    ...
    if(!ic.bSecondaryStructure) {
      me.deferredSecondary = $.Deferred(function() {
          ...
          //var url = "https://dev.ncbi.nlm.nih.gov/Structure/mmcifparser/mmcifparser.cgi";
          for(var i = 0, il = struArray.length; i < il; ++i) {
               var pdbStr = '';
               pdbStr += me.getPDBHeader(i);

               var atomHash = {};
               var chainidArray = ic.structures[struArray[i]];

               for(var j = 0, jl = chainidArray.length; j < jl; ++j) {
                   atomHash = ic.unionHash(atomHash, ic.chains[chainidArray[j]]);
               }

               pdbStr += me.getAtomPDB(atomHash, undefined, true);
          ...
          $.when.apply(undefined, ajaxArray).then(function() {
              me.parseDsspData(arguments, struArray);
          })
          .fail(function() {
              me.parseDsspData(arguments, struArray);
          });
...
iCn3DUI.prototype.parseDsspData = function(data, struArray) { var me = this, ic = me.icn3d; "use strict";
