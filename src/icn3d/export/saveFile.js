...
class SaveFile {
  ...
  saveFile(filename, type, text, bBlob) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let thisClass = this;
      ...
      for (let i = 0, il = ic.commands.length; i < il; ++i) {
        ...
        if (i == il - 1) {
          ...
          command =
            command_tf[0] +
            "|||" +
            ic.transformCls.getTransformationStr(transformation);
        }
      ...
      if (me.utilsCls.isIE()) {
        ...
        if (bAddURL) {
          ...
          reader.onload = function (e) {
            ...
            blob = me.convertTypeCls.getBlobFromBufferAndText(
              arrayBuffer,
              text
            );
  ...
  saveBlob(blob, filename, bBlob, width, height) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    if (bBlob) {
      ...
      let bTooLong =
        url.length > 4000 || url.indexOf("http") !== 0 ? true : false;
      /*
...
*/
      ...
      if ($("#" + ic.pre + "fullscreen").length > 0)
        $("#" + ic.pre + "fullscreen").hide();
  ...
  saveSvg(id, filename, bContactmap) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    if (me.bNode) return "";
  ...
  getSvgXml(id, width, height, bContactmap) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    if (me.bNode) return "";
    ...
    let head =
      viewbox +
      ' title="graph" xmlns:xl="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg" xmlns:dc="http://purl.org/dc/elements/1.1/">';

    //if you have some additional styling like graph edges put them inside <style> tag
    let style =
      "<style>text {font-family: sans-serif; font-weight: bold; font-size: 18px;}</style>";
  ...
  savePng(id, filename, bContactmap) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    if (me.bNode) return "";
  ...
  exportCustomAtoms(bDetails) {
    var ic = this.icn3d,
      me = ic.icn3dui;
    let html = "";
    let nameArray =
      ic.defNames2Residues !== undefined
        ? Object.keys(ic.defNames2Residues).sort()
        : [];
    ...
    nameArray =
      ic.defNames2Atoms !== undefined
        ? Object.keys(ic.defNames2Atoms).sort()
        : [];
    for (let i = 0, il = nameArray.length; i < il; ++i) {
      let name = nameArray[i];
  ...
  exportResidues(name, residueArray, bDetails) {
    var ic = this.icn3d,
      me = ic.icn3dui;
    let html = "";
    ...
    if (residueArray.length > 0) {
      if (bDetails) {
        ...
        for (let chainid in chainidHash) {
          let resStr =
            chainidHash[chainid].length == 1 ? "residue" : "residues";
          html +=
            chainid + " (" + chainidHash[chainid].length + " " + resStr + "): ";
  ...
  printPrevSecondary(bHelix, bSheet, prevRealSsObj, ssCnt) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    let ssText = "";
    ...
    if (prevRealSsObj) {
      if (bHelix) {
        let helixType = 1;
        ssText +=
          prevRealSsObj.resn.padStart(5, " ") +
          prevRealSsObj.chain.replace(/_/gi, "").substr(0, 2).padStart(2, " ") +
          prevRealSsObj.resi.toString().padStart(5, " ") +
          "  " +
          helixType +
          ssCnt.toString().padStart(36, " ") +
          "\n";
      } else if (bSheet) {
        let sense = 0;
        ssText +=
          prevRealSsObj.resn.padStart(5, " ") +
          prevRealSsObj.chain.replace(/_/gi, "").substr(0, 2).padStart(2, " ") +
          prevRealSsObj.resi.toString().padStart(4, " ") +
          "  " +
          sense +
          "\n";
      }
  ...
  getAtomPDB(
    ...
    for (let i in ic.chemicals) {
      ...
      if (atom.elem == "P") {
        ...
        for (let j = 0, jl = atom.bonds.length; j < jl; ++j) {
          ...
          if (serial && ic.atoms[serial].elem == "O") {
            // could be null
            phosOHash[serial] = 1;
          }
        }
    ...
    */
    ...
    let helixStr = "HELIX",
      sheetStr = "SHEET";
    let bHelixBegin = false,
      bHelixEnd = true;
    let bSheetBegin = false,
      bSheetEnd = true;
    ...
    for (let i = 0, il = ssArray.length; i < il; ++i) {
      ...
      if (ssObj.ss != prevSs) {
        // print prev
        stru2header[stru] += this.printPrevSecondary(
          bHelix,
          bSheet,
          prevRealSsObj,
          ssCnt
        ...
        if (ssObj.ss != " ") {
          if (ssObj.ss == "H") {
            ...
            stru2header[stru] +=
              helixStr.padEnd(15, " ") +
              ssObj.resn.padStart(3, " ") +
              ssObj.chain.replace(/_/gi, "").substr(0, 2).padStart(2, " ") +
    ...
    for (let chainid in ic.chainMissingResidueArray) {
      ...
      for (
        let i = 0, il = ic.chainMissingResidueArray[chainid].length;
        i < il;
        ++i
      ) {
        let resi = ic.chainMissingResidueArray[chainid][i].resi;
        let resn = me.utilsCls.residueAbbr2Name(
          ic.chainMissingResidueArray[chainid][i].name
        );
    ...
    let chainIndex = 0,
      fakeChain = "",
      chainNameArray = "abcdefghijklmnopqrstuvwxyz0123456789";
    ...
    for (let i in atomHash) {
      ...
      if (atom.structure != prevStru) {
        ...
        // add header
        let mutantInfo = chainResi2pdb
          ? "Mutated chain_residue " + Object.keys(chainResi2pdb) + "; "
          : "";
        if (!bNoHeader) {
    ...
    */
      ...
      if (!isNaN(atomName.substr(0, 1)))
        atomName = atomName.substr(1) + atomName.substr(0, 1);
    ...
    */
      ...
      if (
        !isNaN(resi) &&
        atom.chain.length > 3 &&
        !isNaN(atom.chain.substr(3))
      ) {
        // such as: chain = NAG2, resi=1 => chain = NAG, resi=2
        resi = resi - 1 + parseInt(atom.chain.substr(3));
      }
      let resiInt = parseInt(resi);
      line +=
        resiInt.toString().length <= 4
          ? resiInt.toString().padStart(4, " ")
          : resiInt.toString().substr(0, 4);
      //line += ' '.padStart(4, ' ');
      // insert
      let lastChar = atom.resi
        .toString()
        .substr(atom.resi.toString().length - 1, 1);
      if (isNaN(lastChar)) {
        line += lastChar;
      } else {
        line += " ";
    ...
    */
        ...
        // use amber atom size
        if (atom.elem == "C") size = 1.908;
        else if (atom.elem == "N") size = 1.824;
        else if (atom.elem == "O") size = 1.6612;
        else if (atom.elem == "H") size = 1.25;
        else if (atom.elem == "S") size = 2.0;
        else if (atom.elem == "P") size = 2.1;
        else if (me.parasCls.vdwRadii.hasOwnProperty(atom.elem)) {
          size = me.parasCls.vdwRadii[atom.elem];
        }
        ...
        if (me.cfg.cid !== undefined && atom.crg !== undefined) {
          ...
          charge = 1.38; // P in phosphate
        } else if (phosOHash.hasOwnProperty(i)) {
          charge = -0.595; // O in phosphate
        ...
        line += atom.b
          ? parseFloat(atom.b).toFixed(2).toString().padStart(6, " ")
          : " ".padStart(6, " ");
        line += " ".padStart(10, " ");
      ...
      if (atom.het && atom.bonds.length > 0) {
        ...
        let bondHash = {};
        for (let j = 0, jl = atom.bonds.length; j < jl; ++j) {
          if (atom.bonds[j] && !bondHash.hasOwnProperty(atom.bonds[j])) {
            // could be null
  ...
  getPDBHeader(struNum, stru2header, mutantInfo, pdbid) {
    let ic = this.icn3d,
      me = ic.icn3dui;
    ...
    if (struNum == 0) {
      let title =
        ic.molTitle.length > 50
          ? ic.molTitle.substr(0, 47) + "..."
          : ic.molTitle;
      // remove quotes
  ...
  showTitle() {
    ...
    let titlelinkColor =
      ic.opts["background"] == "black" ? me.htmlCls.GREYD : "black";
    ...
    if (ic.inputid === undefined) {
      ...
      if (structureArray.length > 1) {
        ...
        for (let i = 0, il = structureArray.length; i < il; ++i) {
          let url =
            isNaN(structureArray[i]) && structureArray[i].length > 5
              ? "https://alphafold.ebi.ac.uk/entry/" + structureArray[i]
              : "https://www.ncbi.nlm.nih.gov/structure/?term=" +
                structureArray[i];
          title +=
            '<a href="' +
            url +
        ...
        //let url = this.getLinkToStructureSummary();
        let url =
          isNaN(structureArray[0]) && structureArray[0].length > 5
            ? "https://alphafold.ebi.ac.uk/entry/" + structureArray[0]
            : "https://www.ncbi.nlm.nih.gov/structure/?term=" +
              structureArray[0];
  ...
  setStructureTitle(url, title, titlelinkColor) {
    var ic = this.icn3d,
      me = ic.icn3dui;
    if (title.length > 40) title = title.substr(0, 40) + "...";
    ...
    if (inputid.indexOf("http") != -1) {
      ...
      text =
        me.cfg.refseqid || me.cfg.protein ? ic.inputid : inputid.toUpperCase();
    ...
    if (!inputid || inputid.substr(0, 4) == ic.defaultPdbId) {
      ...
      let blast_rep_id = me.cfg.oriBlast_rep_id
  ...
  getLinkToStructureSummary(bLog) {
    var ic = this.icn3d,
      me = ic.icn3dui;
  ...
  setEntrezLinks(db) {
    ...
    if (structArray.length === 1) {
      ...
      me.htmlCls.clickMenuCls.setLogCmd(
        "Entrez " + db + " about PDB " + structArray[0] + ": " + url,
        false
      );
      let urlTarget =
